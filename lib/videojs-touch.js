  ...
  */
  var touch = function(controls,data,drawing,debug,options) {
    ...
    var setAspectRatio = function() {
      ...
      if(player.el().parentNode.id==undefined){
        var overlay = document.createElement('div');
        this.draw=SVG(document.getElementById('player1').$.drawing).size(videoWidth,videoHeight);
        var draw=this.draw
        var drawing=document.getElementById('player1').$.drawing;
        ...
        document.getElementById('player1').hammer = new Hammer.Manager(overlay,hammerOptions);

        var hammer=document.getElementById('player1').hammer;
        ...
        var overlay=parent;
        var touchLayer = document.getElementById('player1').$.drawing.parentNode;
        var circle=this.draw.circle(this.height/20).fill({color:'#f06',opacity:0});
      ...
      */
      ...
      for (var i = 0; i < eventList.length; i++) {
        ...
        document.getElementById('player1').hammer.on(eventType, _normalizeAndTrigger(event));
  ...
  // register the plugin
  videojs.registerPlugin('touch', touch);
